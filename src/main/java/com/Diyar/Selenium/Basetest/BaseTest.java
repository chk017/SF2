...
/**
 * This is the base class where driver object, reporting object are defined based on the browser provided in the Config.properties.
 * There are also other objects, values that are initialized. Every test script  class should extend BaseTest.
...
//	public static Duration iTimeout = Duration.ofMinutes(30);
...
//	public static ATUTestRecorder recorder;
	...
	/**
	 * This will read the values from properties file, here it is defined as Config.properties
	 * This method reads the values of url, browser, Timeouts
	 * @author (chk017) kaja ChennnakesavaRao Bachu
	 */
	public BaseTest() {
...
//				iTimeout = Duration.ofMinutes(Integer.parseInt(p.getProperty("waitSeconds_BeforeException")));
	...
	protected void initialize() {
		...
		if(boolVideoRecording) {
			/*
			 * try { Util.createFolder(sRecordingsPath); recorder = new
			 * ATUTestRecorder(sRecordingsPath, this.getClass().getSimpleName() +
...
//		System.out.println("get xml test keys Iter next :" + context.getCurrentXmlTest().getAllParameters().keySet().iterator().next());
		...
		if(boolVideoRecording) {
			/*
			 * try { recorder.start(); } catch (ATUTestRecorderException e) { // TODO
...
//			Reporting.fail("Test : "+result.getName()+" is Failed  with the instance : " + result.getInstanceName());
	...
	protected void closeActiveBrowser() {
		if(boolVideoRecording) {
			/*
			 * try { recorder.stop(); } catch (ATUTestRecorderException e) { // TODO
			 * Auto-generated catch block e.printStackTrace(); }
			 */
			}
